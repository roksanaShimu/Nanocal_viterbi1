#ifndef __PARALLEL_IMPLEMENTATION_1ii_HPP
#define __PARALLEL_IMPLEMENTATION_1ii_HPP


//m  read: page lock memory
...
texture<float, cudaTextureType1D, cudaReadModeElementType> texPmFloat;
__constant__ float d_ev[4];


__global__ void kernel_calculation(unsigned * m_curr_unsigned, float * m_curr_float, unsigned * devPtr_n_fv_unsigned, float * devPtr_n_fv_float, unsigned n_states, float * devPtr_m_float ){
	...
	for(unsigned k=0;k<length_of_from_v;k++){
		...
		const unsigned& j_prev = devPtr_n_fv_unsigned[group_no*warp_size*length_of_from_v + warp_size*k + id%warp_size];
                ...
                float v = log_pr_transition + devPtr_m_float[j_prev];
	...
	__shared__ float level_stdv_or_sd_mean[threads_per_block]; 
	level_stdv_or_sd_mean[threadIdx.x]=tex1D(texPmFloat,pm_index+warp_size);  //level_stdv
	__syncthreads();
	/*

	__shared__ float level_stdv_or_sd_mean[threads_per_block]; //the 3rd element of pm : pm_index+2*warp_size
	sd_mean[threadIdx.x]=tex1D(texPmFloat,pm_index+2*warp_size);

	__shared__ float level_stdv[threads_per_block]; //the2nd element of pm : pm_index+2*warp_size
	level_stdv[threadIdx.x]=tex1D(texPmFloat,pm_index+warp_size);
	*/

	

	float log_level_stdv=log( level_stdv_or_sd_mean[threadIdx.x] );     

	float log_2pi = log(2.0 * M_PI);
	float a=(d_ev[0] - tex1D(texPmFloat,pm_index) ) / level_stdv_or_sd_mean[threadIdx.x];

	m_alpha +=- log_level_stdv - (log_2pi + a * a) /static_cast< float >(2.0);  //log_normal_pdf

	__syncthreads();
	level_stdv_or_sd_mean[threadIdx.x]=tex1D(texPmFloat,pm_index+2*warp_size);  //sd_mean

	float log_stdv=log(d_ev[1]);
	float sd_lambda = pow(level_stdv_or_sd_mean[threadIdx.x], 3.0f) / pow(tex1D(texPmFloat,pm_index+3*warp_size), 2.0f);
	float log_sd_lambda=log(sd_lambda );     // do i need to scale them?

	
	a = (d_ev[1] - level_stdv_or_sd_mean[threadIdx.x]) / level_stdv_or_sd_mean[threadIdx.x];
	m_alpha += (log_sd_lambda - log_2pi - static_cast< float >(3.0) * log_stdv - sd_lambda * a * a / d_ev[1]) / static_cast< float >(2.0);   //log_invgauss_pdf
...
void update_the_m_ii(unsigned * h_m_curr_unsigned, float * h_m_curr_float, vector<Matrix_Entry> & m, unsigned event_number, unsigned n_states){
...
void parallel_calculations_1ii(unsigned n_states, vector<Matrix_Entry> & m , vector< vector<pair<unsigned, float> > > neighbors_from_v, vector<event> ev, vector<pore_model_state> pm){
	...
	*/
	...
	//allocate page lock read_only memory for m(i-1)(n_states): m_pre
	...
	float *devPtr_m_float=assign_page_locked_memory_float(m_float, n_states*sizeof(float));
	//unsigned *devPtr_m_unsigned=assign_page_locked_memory_unsigned( m_unsigned, n_states*sizeof(unsigned));
...
//********************************************************************************
	...
	//allocate texture memory for pm //event independant   : pm
	...
	allocate_texture_memory_float(texPmFloat, pm_rearranged, pm_rearranged_size, n_states*4);
...
//********************************************************************************
	...
	for(unsigned i=1; i<n_events; i++){
		...
		//assign m_pre
		devPtr_m_float=assign_page_locked_memory_float(m_float, n_states*sizeof(float));
		
		//clock_t kernel_start = clock();
		//call kernel (m_pre, m_curr, n_from_v, pm, ev)
		kernel_calculation<<< number_of_blocks, threads_per_block>>>(m_curr_unsigned, m_curr_float, devPtr_n_fv_unsigned, devPtr_n_fv_float, n_states, devPtr_m_float );
		...
		update_the_m_ii(m_unsigned, m_float, m, i, n_states);
		
		cudaHostUnregister(m_float);
